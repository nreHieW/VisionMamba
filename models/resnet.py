    ...
    def forward(self, x):
        ...
        if self.is_downsample:
            if self.identity_method == "A":
                residual = F.pad(
                    x[:, :, ::2, ::2], (0, 0, 0, 0, 0, self.out_channels - x.shape[1])
                )
...
def _check_impl():
    for model_name, config in IMAGENET_MODEL_PARAMS.items():
        ref_model = torch.hub.load(
            "pytorch/vision:v0.10.0", f"resnet{model_name}", pretrained=True
        )
        ref_model.eval()
        ...
        model.eval()
        ref_model_n_params = sum(
            p.numel() for p in ref_model.parameters() if p.requires_grad
        )
        model_n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        assert (
            ref_model_n_params == model_n_params
        ), f"Ref model has {ref_model_n_params} params, model has {model_n_params} params"
        ...
        for (model_n, model_p), (ref_n, ref_p) in zip(model_params, ref_params):
            assert (
                model_p.shape == ref_p.shape
            ), f"Model param {model_n} ({model_p.shape}) does not match ref param {ref_n} ({ref_p.shape})"
